...
def build_parser() -> argparse.ArgumentParser:
    ...
    # A4 — a avaliação final é entrada explícita do chamador, nesta tentativa.
    tf.add_argument("--decision", choices=("new","revise","challenge","no-op"), required=True, help="final retention decision for this attempt")
    tf.add_argument("--assessment", required=True, help="justification for the final retention decision")
    tf.add_argument("--case", help="case the final decision refers to, when there is one")
    ...
    rec = commands.add_parser("recover", help="complete or classify unresolved knowledge mutations")
    rec.add_argument("--operation", help="reconsider exactly this operation, including one already classified uncertain")
...
def dispatch(args: argparse.Namespace) -> tuple[Any, int]:
    ...
    if args.command == "task":
        ...
        if args.task_command=="finish":
            avaliacao={"decision":args.decision,"summary":args.assessment,"case_id":args.case,"covers":args.covers}
            return finish(root,args.id,args.outcome,assessment=avaliacao),0
    ...
    if args.command == "recover": return mutation.recover(root, operation_id=args.operation),0
    if args.command == "retention":
        ...
        return retention_record(root,args.task,args.decision,args.summary,args.case,args.covers),0
