#!/usr/bin/env python3
"""Legacy deterministic schema-1 OperationalCase store preserved by Book V1.
...
def validate_reference(reference: Any, where: str) -> None:
    ...
    if reference.startswith("book:") and CASE_ID_RE.fullmatch(reference[5:]):
        return
    if reference.startswith("event:") and re.fullmatch(r"E-[A-Z0-9]{12,32}", reference[6:]):
        return
